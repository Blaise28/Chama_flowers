...
class carousel_touch_plugin {
  ...
  constructor(carousel) {
    ...
    carousel.contener.addEventListener(
      "touchstart",
      this.start_drag.bind(this)
    );
  ...
  drag(e) {
    ...
    if (this.origin) {
      ...
      this.carousel.translate(
        base_translate + (100 * translate.x) / this.cont_width
      );
    }
  }
  ...
  end_drag(e) {
    if (this.origin && this.last_translate) {
      ...
      if (
        Math.abs(this.last_translate.x / this.carousel.carousel_width) > 0.2
      ) {
...
class carousel {
  ...
  constructor(element, options = {}, images) {
    ...
    if (this.options.loop && this.options.infinite) {
      throw new Error(
        "un carousel ne peut etre a la fois en boucle et enfinie"
      );
    }
    ...
    if (this.options.infinite) {
      ...
      if (this.offset > children.length) {
        console.error(
          "vous n'avez pas assez d'element dans le carousel",
          element
        );
      }
      this.items = [
    ...
    if (this.options.infinite) {
      this.contener.addEventListener(
        "transitionend",
        this.reset_infinite.bind(this)
      );
    }
  ...
  goTo(index, animation = true) {
    if (index < 0) {
      ...
      (this.items[this.currentItem + this.slidesToVisible] === undefined &&
